...
public class TESRPlate extends TileEntitySpecialRenderer<TileEntityPlate>{
	...
	public void render(TileEntityPlate te, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
				...
				GL11.glPushMatrix();
				if(te.angle == 0){
					GlStateManager.translate(x+.5, y+.1,  z+0.05);
				}
				if(te.angle == 90){
					GlStateManager.translate(x+1, y+.1,  z+0.5);
				}
				if(te.angle == 180){
					GlStateManager.translate(x+.5, y+.1,  z+1);
				}
				if(te.angle == 270){
					GlStateManager.translate(x+.05, y+.1,  z+0.5);
				}
